...
for publisher in publishers:
    ...
    for e in range(number_of_experiments):
        ...
        for t in range(T):
            ...
            for A in range(N_ADS):
                ...
                print(" advertisers[A].sampled_weights")
                print( advertisers[A].sampled_weights)
            ...
            new_reward.append(advertisers[0].sampled_weights[superarm[0][0]])
            print("CTS Step 3\n")
            # 3. UPDATE BETA DISTRIBUTIONS
            cts_learner.update(superarm, reward)

        # collect results for publisher
        cts_rewards_per_experiment.append(new_reward)
        print("cts_rewards_per_experiment")
        print(cts_rewards_per_experiment)
    # Plot graphics
    ...
    opt = np.float64(1.0)  # TODO understand how do we obtain opt. I'm sure we have to look at constant q_ij
    plt.figure(1)
    plt.xlabel("t")
    plt.ylabel("Regret")
